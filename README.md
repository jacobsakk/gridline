...
Weekly individual player stats tracker for NAIA, JUCO, NCAA Division II, NCAA Division III, NCAA FCS,
and NCAA FBS football.
...
- `frontend/` — React + Vite front-end (`frontend/src/Gridline.jsx`, ported directly from the
  ...
  conference/position filters, sortable columns. **All six divisions are real, live data** —
  every player, all conferences. No sample/mock data remains anywhere in the app.
- `frontend/src/data/real-stats.json` — the real data for all six divisions, generated by
  `scraper/build_data.py`. Refreshed weekly by GitHub Actions (see "Weekly GitHub Actions job"
  below).
- `scraper/` —
  - `ncaa_api.py` — **done.** Fetches all individual stat leaders (passing/rushing/receiving/
    defense) for FBS, FCS, D2, and D3 from [henrygd/ncaa-api](https://github.com/henrygd/ncaa-api).
    Defense merges 5 leaderboards into one table: tackles, TFL, passes defended, interceptions,
    and sacks.
...
- **D3 conference tags: not built yet, tagged "Independent" for everyone.** D3's standings
  endpoint is broken the same way D2's is (confirmed directly), but D3 has roughly 250 football
  programs — too many to hand-verify against Wikipedia in one sitting the way D2's 159 were.
  `D3_KNOWN_CONFERENCES` in `scraper/ncaa_api.py` starts empty and is meant to be filled in
  incrementally, same technique as D2. D3 also isn't in the conference cross-referencing pipeline
  yet (`scraper/conference_sites.py` has no D3 domains mapped) — real stats and real players
  either way, just without conference grouping or the "beyond the national top ~150" supplemental
  players for now.
...
- [x] D3 wired to real, live NCAA data too (1,860 rows, 4 stat-category tabs) — real players and
      stats today; conference tagging (`D3_KNOWN_CONFERENCES`) starts empty and is a follow-up,
...
- [ ] D3 conference mapping and conference cross-referencing — same two follow-ups D2 already
      got, not yet done for D3's ~250 teams (see "Known limitations").
